...
{
	return (*midi->time_proc)(midi->time_info);
}
...
/*
    ...
    // if latency is zero and this is an output, there is no time reference and midi_synchronize should never be called
...
readProc(const MIDIPacketList *newPackets, void *refCon, void *connRefCon)
{
    ...
    for (packetIndex = 0; packetIndex < newPackets->numPackets; packetIndex++) {
        event.timestamp = packet->timeStamp * 1e-9;	// store in seconds not nanoseconds
//		event.timestamp=now;
...
midi_write_short(PmInternal *midi, PmEvent *event)
{
    ...
    when_ns = (UInt64) ((when + midi->latency) * 1e9) + m->delta;
